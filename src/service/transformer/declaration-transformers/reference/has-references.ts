...
export function hasReferences (node: Node, usedExports: Set<string>, sourceFile: SourceFile, cache: IReferenceCache, seenNodes: Set<Node> = new Set()): boolean {
	...
	if (exportsContainsIdentifiers(identifiers, usedExports)) {
		returnValue = true;
	}
...
function childReferencesAnyIdentifier (child: Node, identifiers: Identifier[]): boolean {
	...
	if (isIdentifier(child)) {
		...
		return (
			(childReferencesAnyIdentifier(child.typeName, identifiers)) ||
			(child.typeArguments != null && child.typeArguments.some(typeArgument => childReferencesAnyIdentifier(typeArgument, identifiers)))
		);
	}
		...
		return (
			child.types.some(type => childReferencesAnyIdentifier(type, identifiers))
		);
	}
		...
		return (
			(child.typeArguments != null && child.typeArguments.some(typeArgument => childReferencesAnyIdentifier(typeArgument, identifiers))) ||
			childReferencesAnyIdentifier(child.expression, identifiers)
		);
	}
		...
		return (
			(child.propertyName != null && childReferencesAnyIdentifier(child.propertyName, identifiers)) ||
			(child.propertyName == null && childReferencesAnyIdentifier(child.name, identifiers))
		);
	}
		...
		return (
			(childReferencesAnyIdentifier(child.elementType, identifiers))
		);
	}
		...
		return (
			(childReferencesAnyIdentifier(child.exprName, identifiers))
		);
	}

	else if (isTypePredicateNode(child)) {
		return (
			(childReferencesAnyIdentifier(child.parameterName, identifiers)) ||
			(childReferencesAnyIdentifier(child.type, identifiers))
		);
	}
		...
		return (
			(childReferencesAnyIdentifier(child.left, identifiers)) ||
			(childReferencesAnyIdentifier(child.right, identifiers))
		);
	}
		...
		return (
			(child.elementTypes.some(type => childReferencesAnyIdentifier(type, identifiers)))
		);
	}
		...
		return (
			(child.type != null && childReferencesAnyIdentifier(child.type, identifiers)) ||
			(childReferencesAnyIdentifier(child.typeParameter, identifiers))
		);
	}
		...
		return (
			(childReferencesAnyIdentifier(child.literal, identifiers))
		);
	}
...
function identifiersAreReferencedByNode (identifiers: Identifier[], node: Node): boolean {
	if (isFunctionDeclaration(node)) {
		...
		return (
			(node.exportClause != null && node.exportClause.elements.some(element => childReferencesAnyIdentifier(element, identifiers)))
		);
	}

	else if (isVariableStatement(node)) {
		return (
			(node.declarationList.declarations.some(declaration => childReferencesAnyIdentifier(declaration, identifiers)))
		);
	}
